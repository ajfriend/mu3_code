...
def corner_leaving_edge(cell: tuple, x: int) -> tuple[tuple, int]:
    ...
    This fold (direction ``1 → 2``) and the corner-name fold in
    ``vertex._normalize`` (``1 → 6``) are the SAME fact — the stitch
    is multiplication by ζ, identifying each pentagon object with its
    ζ-image — read in two coordinate systems. Pinned as one law in
